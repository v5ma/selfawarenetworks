...
Regarding the article below, "Internal Brain State," the reference points to top-down control. A possible process for this control is described in note a0007z.loop as sixth-layer pyramidal-neuron tuning: when signals are sent from sixth-layer pyramidal neurons through the thalamic loop and back down to the somas of pyramidal cells in layers 2 and 3, those somas are excited and the signal-to-noise ratio changes, making the cells more sensitive. The reverse situation could happen if inhibitory interneurons inhibit incoming signals, reducing sensitivity or changing the signal-to-noise ratio. Regular whisking desynchronizes the LFP (local field potential) and EEG (electroencephalogram), which is what sensory input is expected to do. In [Self Aware Networks](gh-a0001z.md) NAPOT theory, the [tonic oscillation](tonic-oscillation.md) absorbs decohering sensory inputs into a synchronized pattern again. The decohering distinctions are the [phase wave differentials](phase-wave-differentials.md) that define the rendering of qualia, meaning the abstraction of whisker feeling as the mouse knows it is rendered in the decoherence pattern.

# "Internal brain state regulates membrane potential synchrony in barrel cortex of behaving mice"
" However, when the mouse is whisking, an internally generated state change reduces the Vm correlation, resulting in a desynchronized local field potential and electroencephalogram. [Action potential](action-potential.md) activity was sparse during both quiet wakefulness and active whisking. Single action potentials were driven by a large, brief and specific excitatory input that was not present in the Vm of neighboring cells. [Action potential](action-potential.md) initiation occurs with a higher signal-to-noise ratio during active whisking than during quiet periods. Therefore, we show that an internal brain state dynamically regulates cortical membrane potential synchrony during behavior and defines different modes of cortical processing."
...
The fact that the thalamus routes information to multiple layers simultaneously is explained with the need for multi-layer [dendritic coincidence detection](dendritic-coincidence-detection.md). Dendrites have a leaky membrane, so signals have to arrive within a short time window, or all at once, to trigger the cell to fire.
...
- Reader route: [San Brain Summary Google Recorder](clusters/san-san-brain-summary-google-recorder-cluster.md) (candidate-routing hub; link added for page-index-backed navigation only).

## Source Provenance

### Original source lineage

- Original source file: `a0296zWhisker.md` in `github.com/v5ma/selfawarenetworks`
- Original source commit SHA: `d1c26a6fbf273695185a4036411215f53209eb71`
- Original source blob permalink: https://github.com/v5ma/selfawarenetworks/blob/d1c26a6fbf273695185a4036411215f53209eb71/a0296zWhisker.md
