...
(tags: synaptogamin calcium dopamine [[multi-vesicular-release|multi-vesicular release]] movement )
...
Another are literally [[neural-oscillations|oscillations]], right? Literally we are oscillating in reaction to the information that we perceive and novel information has a greater impact, then let them less interesting information. So it so if 

Right? Because because you are like electromagnetically as an [[oscillator-consciousness-threshold|oscillator]]. You're sending out more of more of a spike more about like a more of like a [[neural-oscillation-hierarchy|high gamma frequency]], right? It's there's more novel information happening. It's a 

 frequency spike and that sets, the pain that like the decay to the cave rate of that spike.
 
05:01

It's gonna set the pace of the, of all, the [[neural-oscillations|oscillators]] that it interacts with, like, because it's gonna, but it's basically it's gonna set the face because it's basically bumping up all of their they're voltages a little bit. And so that's going to result in basically, and increase chances of [[coincidence-detection-neural-bit|coincident firing]], like, you'll have, like, like you have a whole row of neurons that go off, just because that one neuron went off just because they were close to firing.
...
Go through these, the neural circuits in a [[cortical-column-oscillatory-transmission|cortical column]] would go through like [[neural-sequence-representation|sequences]] of of oscillatory firing. So like it's like a multi-stage, temporal spatial firing event and which you have which you have, but it's like so you have a row with individual neurons firing and then that when that rose done in another row you're going to have another arrangement of neurons that are going to fire because those are the ones that were close to threshold.
...
You don't notice the, the lines being created one at a time. And and that sort of like what I'm suggesting here is that we have we have a sort of like you have a sort of like [[cortical-column-as-display|monitor]] basically the
...
So we're taking this pattern. Okay, that consists of input and input and output for many different. [[synaptic-computation-ltp|Synapses]] the ones that were activated in the ones that were present the two features combined to sort of connect short-term memory with long-term memory.

09:19

And so, I mean, it turns out that the the this synapses are going to change the [[action-potential|action potential]] because they're going to primarily because of the changes to the [[synaptic-ion-channel-tone|potassium channels]]. So when the potassium, if you have more potassium channels, open then the patented potassium moves faster out of the cell body and that could either inhibit the cell or it could repolarize yourself faster.

10:13

So effectively like the potassium helps, shut down the action until so the sodium opens up the [[action-potential|action potential]] and I'm potassium sort of closes the [[action-potential|action potential]]. And the idea is that the [[action-potential-magnitude-via-duration|length of the action potential]] which can vary based upon the potassium channels changes the the [[multivesicular-release-spatial-information|multi vesicle release pattern]] so you have between zero.
...
This means that the quantity of [[dopamine-effort-motivation|dopamine]] or acetylchoine or GABA release can be ramped up or ramped down depending on the duration of the [[action-potential|action potential]].
...
The exit terminal, the fluids in the next terminal. I guess are increasing at that point. So when the best of goal when the vessel and the vesicle is going to land on, if it lands on a on a [[synaptic-computation-ltp|synapse]], on the other side, that's going to add some some mechanical force and some mechanical force that leads to [[neuron-electrical-firing-threshold|electrical stimulation]] and the the channel will allow sodium ions or out in or potassium ions out.

13:24

And I think that, so, I think that if you get like a really large [[action-potential|action potential]], you're going to that. I think that it's going to inhibit the next sound like as downstream [[gh-a0206z|self]]. I think it's gonna have it is downstream cells because it's gonna send out a like a number three, three [[multi-vesicular-release|micro vesicles]] all at once.

13:54

And that means that all that, that likely, that all of the potassium is opened up at once. All the testing channels are opened over once is basically going to inhibit the cell and that's scenario. And so, so like when you're on spikes really hard, it's gonna like a [[neural-phase-pattern-propagation|phase of spike]], it's gonna open up all the potassium channels because is with the microgressive hole with a with the multi micro vessel release.
...
But let's say that that the that you know, neurons intersect was a couple in each layer that fire and then in the next there's a couple of each other image layer that part because it's like I'm going with this bar distributor representation theory, you know presented by that it was presented so well by Jeff Hawkings and the folks at momentum and many papers and two books and so I subscribed to this [[pattern-propagation-neural-layers|project distributed representation]] idea that has what the brand is doing and it appears to be correct.
...
They're threshold. And then that might then cause some other group of neurons somewhere to start hiring and but imagine that each stage you see, these neurons are firing in. In terms of as they move up, the [[fractal-brain-scale-hierarchy|hierarchy]] of of the cortical column, the neurons are firing in [[neural-sequence-representation|sequences]] and some of the sequences, you might see like a diagonal line or you might, you might be able to draw lines between them, that, like, have this sort of zigzagging almost vector like graphical quality, right?
...
And I what I mean by that, is that as as the over time human beings have grown increasingly greater. Okay, so I am presuming as this is true, but I would presume that over time. Human beings have evolved to have an increasingly diverse, and increasingly genetically, diverse arrangement of, of [[neuron-rank-protein-synthesis|neurons]].

17:56

And of, you know, with different like electro, acoustical properties. And and mechanical properties and different like, you know, you can see an example of this. I guess if you look and how human [[dendritic-pattern-detection|dendrites]] are much longer than mouse bend rights, right?
...
Like imagine like a dome it has because of its reflection because of its reflecting shape, if a dome was designed to reflect them [[magnetic-phase-field|electromagnetism]], you know, maybe some very some, some kind of material that you make a dome out of, right? And then, so what would it be? Like then to like deliberately try to concentrate electromagnetism with with a dome that reflected energy very well.

21:35

That's an interesting. It's an interesting thought. I don't know. But like imagine that. So like your brain at your skull has, it's kind of like a dumb, you know, like your eyeball, like imagine your your brain and your eye are our [[gh-map-intro-fractal|fractals]] of one. Another like you're I mean you're your school like your skull is it is kind of like a it's on it's almost it's it's almost a [[functional-fractal-isomorphism|dome]] you know it's round it encompasses your goes around your brain and protects it from every direction.
...
Anyway, so we have. So we have this, so we have this dome and our head, and it's weird to like well, so what, so if our brain on some level is producing an [[magnetic-phase-field|electromagnetic phase field]] and in, I think I can make the argument that space time and tell this technically a face field.

23:05

And I'm arguing that basically is the best field. Is that particles are [[periodic-physics|phases]], particles are frequencies and they're frequencies between distances.
...
- Source file: `b0114y.md` in github.com/v5ma/selfawarenetworks
- Commit SHA: `d1c26a6fbf273695185a4036411215f53209eb71`
- Blob permalink: https://github.com/v5ma/selfawarenetworks/blob/d1c26a6fbf273695185a4036411215f53209eb71/b0114y.md
