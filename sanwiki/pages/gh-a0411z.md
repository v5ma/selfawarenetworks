...
I mentioned [[category-theory|category theory]]. I want to sort of collect and review everything I have that is about [[category-theory|category theory]]. So I want to add category to the [[napot-legacy-wiki-map|map]] of all things

[[micah-blumberg|Micah Blumberg]]
Regarding [[artificial-intelligence|Artificial Intuition]] by Monica Anderson, and Grok by Numenta: Is a [[semantic-tokens-as-vector-geometry|semantic token]] the [[consciousness-as-information|meaning]] of a word?
Is a [[semantic-tokens-as-vector-geometry|semantic token]] a map of [[vector-embeddings|vectors]] between points above a certain threshold in various nodes?
Is a [[semantic-tokens-as-vector-geometry|semantic token]] like a matrix, that can be understood as a spatial metaphor, or a description in [[category-theory|category theory]] that defines or remembers a relationship between [[vector-embeddings|vectors]] that emerge in space?
Is model free [[artificial-intelligence|intelligence]] about creating a [[learned-spatial-model|model]] based on whatever emerges beyond the thresholds that filter significant [[coincidence-pattern-neural-information|patterns]] from noise?
I'm getting a new sense of the inner [[multi-modal-sensory-array-geometry|geometry]] of my [[brain-as-sensory-processor|mind]] and the geometry of my thoughts.

See this video http://medicalxpress.com/news/2012-10-fetal-neural-vision.html this does not look like single character bits fed one at a time triggering points converted into [[vector-embeddings|vector]] mapped based memories. It looks like a morphing [[vector-embeddings|vector]] [[coincidence-pattern-neural-information|pattern]] going through several stages of [[attractor-memory|memory]] reduction in the [[neural-circuits|architecture]]. Essentially because the burst runs out of steam, and then it leaves its [[vector-embeddings|vector]] mapped imprint. A [[artificial-perception|spatial representation]] imprinted in the fired connections.

I hope replies reflect the subject of this group: [[category-theory|Categories]] & Geometry of the Mind. Do you think meaning for intelligent beings may imply an unconscious [[tomographic-map-of-reality|topographical map]] of nodes that are themselves [[semantic-tokens-as-vector-geometry|semantic tokens]] reflecting maps between points of above threshold data associations in a multi level [[hebbian-learning|pattern learning]] machine?

Burst of fetal neural activity necessary for vision
medicalxpress.com
(Medical Xpress)—A sudden and mysterious burst of activity originating in the [[retinal-oscillations|retina]] of a developing fetus spurs brain connections that are essential to development of finely-tuned sight, Yale researchers report in the journal Nature. Interference with this spontaneous wave of activity could play .....
...
[[micah-blumberg|Micah Blumberg]] Just imagine that when you see something, this is happening in your [[brain-as-sensory-processor|brain]]. Imagine that you're having [[cortical-traveling-waves-san-submission|waves of sight]] pass through your [[cortex|brain tissue]], flowing in almost like water with special properties.

It's not like feeding binary bits into a [[hebbian-learning|learning]] machine one at a time, it's like pouring in a data stream, words, pictures, bits, all jumbled together, and letting the structure sort it all out.
October 11 at 2:26pm · Like

[[micah-blumberg|Micah Blumberg]] The question I would have for Monica, and or [[jeff-hawkins|Jeff Hawkins]], would be, can you visualize the activity in your computer based [[connectome]] with graphics so that the activity looks like this. Would the activity look like this at all? Perhaps your visualized [[connectome]] would not have [[cortical-traveling-waves|brainwaves]]?
October 11 at 2:35pm · Like

[[micah-blumberg|Micah Blumberg]] Do your [[semantic-tokens-as-vector-geometry|semantic tokens]] ([[vector-embeddings|vector]] maps of associations) really come from emergent [[coincidence-pattern-neural-information|patterns]] that are imprints in the [[attractor-memory|memory]] created by synthesized [[cortical-traveling-waves|brainwaves]]?
October 11 at 2:36pm · Like

[[micah-blumberg|Micah Blumberg]] I like how in AN model, [[layer-6-inhibitory-intuition|intuition]] is the emergent result from points plotted in a [[vector-embeddings|vector]] tree map that you can describe in [[category-theory|category theory]] (the [[vector-embeddings|vector]] tree map not the emergent [[coincidence-pattern-neural-information|patterns]]). I think she describes it in C and or Java, but I think C and Java lack the fluidity that could happen in a functional language like Haskell. Where node memories can create [[vector-embeddings|vector]] memories and the model free nature means that the concept is the [[attractor-memory|memory]] of the tree's reaction in a sense. With Haskell and an 80 core processor one could have each node in the tree creating emergent [[vector-embeddings|vector]] maps with the other activated nodes independent of a centralized process.

I like how in AN Model, the "concept" isn't pre-thought and pre-programmed, it's learned or built by the [[vector-embeddings|vector]] tree's [[attractor-memory|memory]] system in response to the [[information-pipeline|stream of data]].
October 11 at 3:20pm · Like · 1

Juan Carlos Kuri Pinto Yup. I think Haskell is the right language for the [[multi-modal-sensory-array-geometry|geometry of mind]]:
...
[[micah-blumberg|Micah Blumberg]] Regarding the film clip again: It's as if the parts of the [[brain-as-sensory-processor|brain]] creating the [[cortical-traveling-waves|brainwave]] are the parts that have reached their threshold point, resulting in a forwarding of the excess of that energy onward. Like water that fills up buckets, and if there is more water than buckets it spills onward, in a [[cortical-traveling-waves-san-submission|wave]] of running water until the remaining amount of water no longer exceeds the threshold of any bucket.
October 11 at 3:42pm · Like

[[micah-blumberg|Micah Blumberg]] The peaks of that [[cortical-traveling-waves|brainwave]] also appear to be spread out as opposed to convergent like you might see in a [[fractal-neural-scale-hierarchy|pyramid like hierarchy]].
October 11 at 3:45pm · Like

[[micah-blumberg|Micah Blumberg]] Think of the edge of a letter, the edge of a box, and the edge of a wall all at the same time. Each of those edges is in a different scale. The smallest you would expect to be a [[vector-embeddings|vector]] token from the lowest part of the [[cortical-plasticity|cortex]]. and the largest would be a [[vector-embeddings|vector]] token from a much higher part of the [[cortical-plasticity|cortex]], even though they are both edges. Then if you try to visualize the edge that represents a piece of the electron's potential orbit around the center of an atom (if that exists even?) and the edge that defines a section of the orbital path that the sun takes through the galaxy. (how do we know our sun isn't leaving the galaxy or headed toward the black hole at the center?) Now we have a situation where the conceptual scale for both the edge of the electrons orbit, and the edge of the suns orbit are so epic conceptually that they probably are both modeled at the highest levels of [[cortical-plasticity|cortex]], unless the association of the edge of an atom is so closely associating with small things that it gets thrown into a [[vector-embeddings|vector]] map that is a link between details from lower regions and concepts from higher religions. Maybe all the edges are size invariant, like vector graphics in a computer. Maybe doing computer [[inside-out-vision-model|vision]] by feeding it bitmaps is creating a disgusting scaling problem in [[inside-out-vision-model|vision]] learning that would not exist if "edges" in the v1 cortex were scale invariant [[vector-embeddings|vector]] [[coincidence-pattern-neural-information|patterns]], that could be linked into any [[vector-embeddings|vector]] token, aka a [[semantic-tokens-as-vector-geometry|semantic token]], that is also linked to a size [[vector-embeddings|vector]] token, and a depth [[vector-embeddings|vector]] token, and a color [[vector-embeddings|vector]] token, and a smell [[vector-embeddings|vector]] token, to produce the appearance of a multi-sensory object in stages of brainwaves.
...
[[semantic-tokens-as-vector-geometry|Semantic tokens]] are indeed the result of emergence.
...
Juan Carlos Kuri Pinto Monica, what do you think about [[qualia|subjective experience]]? (i.e. red color, rose scent, emotional pain, etc.) Will it emerge from the right [[connectome]] algorithm or is it an inherent property of biological neurons?
...
Gödel's incompleteness theorem BREAKS when your axiom system refers to Reality! The vicious cycle of [[gh-a0206z|self]]-references is broken by R...See More
...
Sometimes I think [[qualia|subjective experience]] is produced by pattern combinations that are descriptions of noumena. Obviously, subjective experience is caused by combinations of electrochemical patterns. There is no significant difference between the electrochemical patterns of vision, audition, taste, olfaction, touch, labyrinth's orientation, and proprioception.

Moreover, evidence suggests the local [[neural-circuits|neural circuits]] in most parts of the [[cortex]] all use the same general purpose learning algorithm:
— The fine-scale anatomy of the cortex looks pretty much the same all over;
— if the visual input is sent to the [[auditory-cortex|auditory cortex]] of a newborn ferret, the "auditory" cells learn to do vision;
...
Now that we know specific [[qualia|subjective experience]] is not a property of some specific brain regions because there is a general purpose learning algorithm, the question is rather reformulated: Is subjective experience an emergent property of such general purpose learning algorithm? In other words, an emergent property of [[integrated-information-theory|integrated information]]. Or is subjective experience an emergent property of electrochemical patterns? If the first hypothesis is correct, we could actually create subjective experience inside machines. Otherwise, if the second hypothesis is the correct one, we will be able to create subjective experience inside machines only if we replicate the crucial aspects of subjective experience at the hardware level, in other words, through imitation of electrochemical patterns.
...
[[gh-a0403z|Introduction]] to Genetics and Evolution
www.coursera.org
[[gh-a0403z|Introduction]] to Genetics and Evolution is a free online class taught by Mohamed Noor of Duke University
...
Juan Carlos Kuri Pinto There is a slight error in Piaget Modeler's diagram: All those cognitive functions are not separated. They are rather fused in [[cortical-columns|cortical columns]] and the [[connectome]].
18 hours ago · Like · 3
...
Monica Anderson Anna, about the need for two input channels... in the brain there are only neurons signaling neurons. Why would it make a difference to any individual neuron whether the signal comes from the eyes or the skin? It can't tell the difference. This is my main argument against the [[embodiment-in-cognition|Embodiment Hypothesis]] - that the "fundamental grey matter algorithm" cannot depend on the availability on multiple input senses for its function.
14 hours ago · Like

Monica Anderson The "Enactment Hypothesis" (that *[[ai-sensory-grounding|interaction]]* with the environment is necessary for learning and intelligence) is also questionable but the argument is longer and I'm not done with it yet. 
14 hours ago · Like
...
[[micah-blumberg|Micah Blumberg]] "The "natural" humans intuitively feel that the words per se don't have absolute meaning,"
...
[[micah-blumberg|Micah Blumberg]] Perhaps all the diagrams in Piaget can be considered part of the substrate Monica ? Isn't it possible to create a substrate that is a super structure underneath and separate from the parts that are open for raw model free machine learning?
9 hours ago · Like

[[micah-blumberg|Micah Blumberg]] The human brain for example does appear to have an organized super structure that cannot be learned via a life's experience. The input from the eyes goes to a specific region ([[visual-cortex|v1]]) and the input from ears goes to another specific region, and those regions do not immediately combine, but they do combine at higher levels. That's a super structure in the substrate that is exterior to the work of a substrate that is just the work of a single learn algorithm. If I am not mistaken. I think you (Monica) mentioned that neuro [[cortical-columns|cortical columns]] are perhaps incidental at this point in your research, but I don't see how that is not [[reductionism-vs-phenomenology|reductionism]] instead of holism in your model. A single algorithm substrate may just be another form of reductionism. You argue that the holism aspect in not creating and pre-structures in the learning area right? How is that not another form of reductionism? How do we conclude that the mind starts out "blank" (without some models in memory, perhaps from random connections, or from genetically passed connections, real general stuff.) without making the same error as all the other reductionists?
9 hours ago · Edited · Like
...
PAM Talk AISB-11
www.scribd.com
Presented at the University of York, April 5th 2011 for the Society of [[artificial-intelligence|Artificial Intelligence]] and Simulation of Behavior (AISB-11) Conference.
...
Pawel A. Pachniewski [[gh-resnote04|Micah]], it doesn't appear to have it, it has it.
about an hour ago · Like

[[micah-blumberg|Micah Blumberg]] what has what?
24 minutes ago · Like
