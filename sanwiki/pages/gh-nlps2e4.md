[[neural-lace-podcast|The Neural Lace Podcast]] Season 2 Episode 4
...
Hi, this is [[micah-blumberg|Micah Blumberg]] with the Neuro Lace Podcast, and I have a special guest here with

me today who's going to talk about functional f-nirs, or [[fnirs-functional-near-infrared-spectroscopy|Functional Near Infrared Spectroscopy]].
...
excited about [[fnirs-functional-near-infrared-spectroscopy|f-nirs]] as a potential tool for [[fnirs-biofeedback|biofeedback]], so he built a device and

tested it out, and he found that it does actually work, that you can control your brain blood

oxygenation. So [[fnirs-functional-near-infrared-spectroscopy|f-nirs]], just for the people who aren't aware, f-nirs is a tool for measuring
...
So something that can be done with [[fnirs-biofeedback|f-nirs is biofeedback]]. Another thing that you can

do with it is pure measurement. So [[fnirs-brain-imaging|f-nirs]] is sort of like the poor man's [[fmri|fMRI]] machine.
...
optimized for the [[fnirs-biofeedback|biofeedback]] application, which is, biofeedback is where you're measuring
...
Okay, so the question was, what can [[fmri|fMRI]] be used for? And it can be used for basically
...
see how heavily they're activated. Basically, [[fnirs-brain-imaging|fMRI is a poor man's fMRI machine]]. So it
...
other application is what I've designed my device for, which is [[fnirs-biofeedback|biofeedback]]. So biofeedback
...
of weeks, where they were measuring the [[auditory-cortex|audio cortex]] and they were having the audio cortex

manipulate something in a computer program. And then the [[gh-a0004z|neural network]] was analyzing
...
Oh, really? Yeah. So it is hard to do that in a useful way. So in the [[fmri|fMRI]] literature

or the [[fnirs-functional-near-infrared-spectroscopy|FNERS]] literature, there's this thing called the hemodynamic response function.
...
and nearly impossible. So to try to read somebody's brain and try to get useful [[brain-computer-interfaces|brain-machine]]
...
with the [[fnirs-brain-imaging|FNERS approach]] or with [[fmri|fMRI]]. But if you do something like implant electrodes
...
about [[eeg|EEG]] and you could get good spatial data if you do like 256 electrode cap. What kind
...
you better [[fnirs-brain-imaging|spatial resolution]] if there was as much engineering effort that had been invested

in [[fnirs-functional-near-infrared-spectroscopy|FNERS]] as has been invested into [[eeg|EEG]]. The reason for this, there's actually a few reasons
...
ability to get spatial resolution just because of that. With [[eeg|EEG]], you end up getting like
...
So for example, the model that I led a team that we brought [[eeg|EEG]] into [[webxr-bci-integration|WebVR]], and I just
...
So you might have a structural [[fmri|MRI]] image that you take of a person's brain, and then you

do your source localization for [[eeg|EEG]], and you overlay that on top of the structural MRI.
...
filter for [[fnirs-brain-imaging|brain imaging]] to take the uncertainties in each of the different measurement methodologies
...
from the sensors and run a [[gh-a0004z|neural network]] and try to look for correlations with the

neural network. Yeah, you can always just run a [[gh-a0004z|neural network]] on it and see what it

comes out with, but then you don't know what it's doing.

See, I would want the [[gh-a0004z|neural network]] to predict reoccurring patterns and to help point out
...
get with Hefner's. So do you subscribe to like the [[semantic-cortical-mapping|semantic map]] outlined at Jacqueline's

labs, like how he's sort of like that research? I don't know how it works exactly across individuals

or [[semantic-cortical-mapping|semantic maps]] drastically different between people and like in terms of what areas of

the [[brain-as-memory-prediction-machine|brain]] are. I'm not sure exactly what you're referring to. Jacqueline's lab was like he

did the [[fmri|fMRI imaging]] and he said, okay, well, you have, you may have like a dog and a cat
...
Yes, I'm not sure what you mean by the [[semantic-cortical-mapping|semantic map]] stuff, but the observation that different

types of objects are represented or encoded in different parts of the [[brain-as-memory-prediction-machine|brain]] is a fairly
...
that is present in an [[action-potential|action potential]] that arrives at a neuron is that the previous neuron
...
by a timing I meant frequency. Like how often the [[action-potential|action potentials]] come. That matters and

also the phase matters. There's a really interesting circuit in the [[auditory-cortex|auditory processing pathway]]
...
And it just shows you that the [[brain-as-memory-prediction-machine|brain]] likes to encode information by increasing the number
...
with us. Social media has allowed us to form these bubbles that are [[gh-a0206z|self]]-reinforcing. When
...
this principle called [[hebbian-learning|heavy in learning]]. Neurons that fire together, wire together. So when

a pre-synaptic neuron fires an [[action-potential|action potential]] that pushes the post-synaptic neuron over its

activation threshold and causes it to fire, that [[synapse|post-synaptic]] neuron goes through some
...
some sort of electromagnetism happening where the [[action-potential|action potential]] is an electrical event
...
above the threshold, it triggers the cell body and the axon to go into an [[action-potential|action potential]],

which is the digital signal. And once that happens, once the [[action-potential|action potential]] is triggered,

all of the downstream neurons will get the same signal. There's no [[receptor-inhibition-pattern|filtering of signals]]. It just
...
Do you buy into the [[predictive-coding|predictive coding]] theory of the idea that your memories are becoming
...
people in [[brain-computer-interfaces|brain-computer interface]] are imagining they can do in terms of reading the mind and
...
And so from that perspective, you can create brains using [[gh-a0004z|neural networks]] where you're just
...
The [[dendrite|dendritic tree]] is very much an analog computer. I mentioned that the neuron as a whole is

digital, but the [[dendrite|dendrites]] are very analog and very sensitive, and they have a lot of different
...
get [[action-potential|action potentials]] within dendrites. The voltage-gated sodium channels that are necessary

for [[action-potential|action potentials]] on the axon are also present in smaller concentrations in [[dendrite|dendrites]]. And so
...
inhibition. You have several different types. You've got, you know, you've got [[neurotransmitter|glutamate]],
...
classes of [[neurotransmitter|glutamate receptors]]. And two of them are purely, instantly excitatory. But a third one
...
store a [[attractor-memory|memory of events]], to help the timing of when a neuron fires and help the decision of

when a neuron fires in response to criteria. I've heard a neuroscientist whose book I read

talk about how a neuron could be considered a [[criteria-causation-single-neuron-pattern-detection|criteria detector]] or a [[coincidence-pattern-neural-bit|coincidence detector]].
...
there's many layers of our [[brain-as-memory-prediction-machine|brain rendering reality]]. Maybe at the baseline it's just like
...
essential to how the [[brain-as-memory-prediction-machine|brain processes information]] and maybe how the brain sort of communicate
...
Yeah, so I think the [[coincidence-pattern-neural-bit|coincidence detector]] is a good starting point. The brain definitely has
...
detection happens. The third [[neurotransmitter|glutamate]] receptor that I sort of alluded to, the [[nmda-receptor|NMDA receptor]],

is actually very much a coincidence detector. It only responds when you have multiple [[action-potential|action potentials]]

in a nearby portion of the [[dendrite]] that happens in a few milliseconds. Yeah,
...
are on the opposite sides of your head, but is there maybe an [[nmda-receptor|NMDA receptor]] that might fire
...
at roughly the same time in order to activate that [[nmda-receptor|NMDA receptor]]. And once that happens,

that [[nmda-receptor|NMDA receptor]] causes that synapse or that set of synapses to become stronger and to be more
...
The [[visual-cortex|visual cortex]] has the structure it does, independent of sensory stimuli. And sensory
...
don't all encode the same coincidence. There's also what we call [[receptor-inhibition-pattern|lateral inhibition]] going on.
...
just the nuts and bolts [[synapse|wiring diagrams]] of which areas receive connections from which other areas
...
[[episodic-memory|episodic memory]]. There's semantic memory. There's procedural memory. There's implicit memory.

There's explicit memory. There's [[working-memory|working memory]]. There's short-term sensory store memory,

both in [[visual-cortex|visual cortex]] where you have the visual spatial sketch pad. And then you've also got

the [[auditory-cortex|auditory cortex]] for the phonological loop, which is about two seconds. So you've got each

of those types of memory. And a lot of those types of memory have different mechanisms that

underlie them. Like the phonological loop, that's probably a dedicated [[gh-a0004z|neural network]] that operates
...
The visual spatial sketch pad is probably something different. [[attractor-memory|Long-term memories]] are also

very different. Those mostly go through, like, the heavy in learning to calcium going through

[[nmda-receptor|NMDA receptors]] to gene expression changes, increases in receptor densities and synapse densities.
...
And one of the reasons why I'm excited about the [[fnirs-biofeedback|Nurentford Spectroscopy Biofeedback]] process
...
do a task or not depending on environmental cues. And you could try to do [[fnirs-biofeedback|biofeedback]] in order to
...
[[fnirs-biofeedback|near and front spectroscopy value feedback]] stuff. Yeah, I've been working on a lot of other projects
...
viable, I think is interesting and exciting. But yeah, like the [[fnirs-biofeedback|biofeedback]] I think has

the potential to help a lot of people. There's some circumstantial anecdotal evidence that indicates

that doing [[fnirs-biofeedback|biofeedback]] on [[prefrontal-cortex|prefrontal]] blood oxygenation may improve concentration and focus,
...
- Source file: `nlps2e4.md` in github.com/v5ma/selfawarenetworks
- Commit SHA: `d1c26a6fbf273695185a4036411215f53209eb71`
- Blob permalink: https://github.com/v5ma/selfawarenetworks/blob/d1c26a6fbf273695185a4036411215f53209eb71/nlps2e4.md
