...
As we add algorithms on top of this signal format model, we have to add more details to the model. If I build a reinforcement learning system (which I claim is required for all AGI). we must decide how to MODEL the reward input. Is it a real number that shows up at unspecified times? Such -infinity to +infinity represented as a 64 bit floating point value? Or a real value from 0 to 1? Or -1 to +1? Or as a binary input with one input for a binary pleasure input, and another for a binary pain input? Or do I only have a single bit pleasure input, and nothing else (like the Facebook "Like" reward signal)? These more model decisions the engineer must make about what to model, and how to model it.
