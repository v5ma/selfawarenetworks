...
Note b0010y is a simple transcription of audio note b0010y about [[neural-array-projection|neural array projection]] and [[cortical-column-oscillatory-transmission|cortical column oscillatory transmission]] found here: https://recorder.google.com/share/6e2e499f-5cf8-4b2f-8e64-e77f44a9b2de

# b0010 and a0310z

b0010 was given a major re-write and turned into note a0310z which also became the bases of the [[neural-array-projection|Neural Array Projection]] Oscillation Tomography Revision 5 video found here: https://youtu.be/vixhppNAKPs

# ChatGPT 4 Single Sentence Summary of both Part 1 & Part 2 Summaries:

[[action-potential|Neurons fire]] at tonic frequencies and use rate coding with varying amplitudes and magnitudes, allowing for selective activation and real-time pattern modification by [[inhibitory-interneurons-plasticity-gating|inhibitory interneurons]], ultimately shaping brain processing and long-term memory development.

# ChatGPT 4 One Line Summary of the first half (200 double spaced lines): 

Rate coding with varying amplitudes and magnitudes in inhibition leads to unique transmission of patterns in neurons through nonlinear computation and [[action-potential-phase-space|phase wave]] changes.
...
Neuron output changes through [[action-potential-phase-space|phase wave]] adjustments.
...
There is an interplay between high phasic bursts and slower tonic bursts in [[gamma-wave-consideration-sandwich|gamma]] and [[theta-wave-reality-generator|theta]] patterns.

Brain activity forms a loop-like structure, with feedback and interactions across various regions, including [[thalamo-cortical-information-flow|thalamo-cortical information flow]].

The timing of back-propagating signals from postsynaptic to presynaptic neurons can be modified by other neurons through [[synapse|synaptic]] regulation.

Neurons represent a part of a pattern, with sparse distributed internal representations being rendered when neurons send signals to other neurons across [[cortical-columns-reference-frames|cortical column reference frames]].
...
Patterns are encoded through a combination of excited and inhibited [[synapse|synapses]].
...
[[thalamus|Thalamic]] feedback loops coordinate brain cells.

[[cortical-traveling-waves|Oscillatory firing]] binds signals across the brain.
...
The brain's computation occurs at the neuron, [[cortical-column|cortical column]], and inter-cortical column levels.

Neural pathways develop as an organism navigates its environment and updates [[cortical-cell-recycling|cortical cell]] patterns.
...
the [[synapse|synapses]] of each neuron. This rate coding with varying magnitudes and individually encoded
...
it could become inhibited for a while, or it could fire at some faster range. And there's,

there's, there's, there are multiple faster ranges. So, so, there's the, there's the basic

[[action-potential|action potential]] that's faster than the tonic [[action-potential|action potential]]. The higher, so the, so the
...
[[action-potential|action potential]] firing that is faster than the tonic [[action-potential|action potential]] of regular firing.

And, and then if that, if that [[action-potential|action potential]] happens again at the same time, then it represents
...
of the soma, are going to supercharge the [[action-potential|action potential]] event if it happens again

at the same time. So, it's like the, the post-saphtic side of the soma gets encoded with the expectation

of when the last speed-up in [[action-potential|action potential]] happens. And the timing of, in that, that
...
tonic burst. For example, there's an interplay between [[gamma-wave-consideration-sandwich|gamma burst]] and tonic [[theta-wave-reality-generator|theta patterns]].
...
the Hedgehog loops. And each [[cortical-column|cortical column]] is also like a loop of loops. So, it's the

loop of like, so each cortical column is like a stack of Sonic the Hedgehog loops in the

formation of a loop. And then at the [[pyramidal-cell|pyramidal cell]], at the fifth and the sixth layers, they're
...
of, of back propagating signals, back propagating [[synapse|synaptic signals]] that are coming from a postsynaptic
...
the, the tonic rate frequency, which, which might be 20, which is like a [[action-potential-magnetic-phase-field|power band frequency]].

Could be 20 Hertz, 40 Hertz, 60 Hertz or 80 Hertz, but it is a regular pattern that a neuron

does. It's just going to, every once in a while, it's going to fire an [[action-potential|action potential]]
...
of the [[pyramidal-cell|pyramidal cell]]. The, the, the cortical columns, the pyramidal cells in the cortical
...
feedback, which is going to excite your, your [[pyramidal-cell|pyramidal cells]] are going to go through the
...
the timing of the synapses on the [[apical-dendrites|apical dendrites]] to so that if you walk across the blue car
...
routed through the thalamic matrix and thalamic core neurons to provide a loop of feedback
...
physics and the [[cortical-traveling-waves|oscillatory firing]] also binds the signals so that they can be so that your
...
signals is the process by which your brain is detecting its own information. So the burst
...
[[action-potential|action potential]] which is the [[slow-cortical-potentials-napot-revision-3|slow wave potential]] which is slower than the tonic frequency but
...
might be an original sequence of neural firing and across, that means a sequence of firing
...
at the neuron level, at the [[cortical-column|cortical column]] level, at the inter, at the inter-cortical
...
- Source file: `original archived b0010y.md` in github.com/v5ma/selfawarenetworks
- Commit SHA: `d1c26a6fbf273695185a4036411215f53209eb71`
- Blob permalink: https://github.com/v5ma/selfawarenetworks/blob/d1c26a6fbf273695185a4036411215f53209eb71/original%20archived%20b0010y.md
