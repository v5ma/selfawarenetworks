b0103y Whisper (Transcribed by OpenAI Whisper)
...
Reader link map: This transcript is primarily about [[brain-rhythms-in-cognition|brain rhythms in cognition]], [[neural-oscillation-hierarchy|neural oscillation hierarchy]], [[oscillating-network-of-oscillators|oscillating networks of oscillators]], [[oscillatory-neural-coupling|oscillatory neural coupling]], [[spontaneous-harmonic-synchronization|spontaneous harmonic synchronization]], [[neural-ensemble-phase-reset-communication|neural ensemble phase reset communication]], [[tonic-phasic-neural-oscillation|tonic-phasic neural oscillation]], [[tonic-oscillation|tonic oscillation]], and [[tonic-oscillation-as-predictive-reference-frame|tonic oscillation as a predictive reference frame]].

The neural mechanism discussion repeatedly returns to [[action-potential|action potentials]], [[action-potential-phase-space|action-potential phase space]], [[action-potential-topology|action-potential topology]], [[action-potential-magnitude-via-duration|action-potential magnitude via duration]], [[neuron-electrical-firing-threshold|neuron electrical firing threshold]], [[pyramidal-cell|pyramidal cells]], [[dendrite|dendrites]], [[dendritic-computation-pattern-detection|dendritic computation pattern detection]], [[dendritic-computation-network-detection|dendritic computation network detection]], [[coincidence-detection-neural-bit|coincidence detection as a neural bit]], and [[coincidence-pattern-neural-information|coincidence-pattern neural information]].

The transcript frames these mechanisms through network-scale topics such as [[sensor-array-consciousness|sensor array consciousness]], [[cortical-column-oscillatory-synchronization|cortical column oscillatory synchronization]], [[cortical-column-oscillatory-collaboration|cortical column oscillatory collaboration]], [[cortical-column-signal-projection|cortical column signal projection]], [[cortical-small-world-topology|cortical small-world topology]], [[thalamo-cortical-information-flow|thalamo-cortical information flow]], [[thalamic-bridge-consciousness-portal|thalamic bridge consciousness portal]], [[attention|attention]], [[attention-schema|attention schema]], and [[attention-schema-theory|attention schema theory]].

Its plasticity and inhibition thread connects [[ltd-inhibitory-pattern-sharpening|LTD inhibitory pattern sharpening]], [[ltp-structural-memory-encoding|LTP structural memory encoding]], [[synaptic-computation-ltp|synaptic computation and LTP]], [[synaptic-connectivity-dynamics|synaptic connectivity dynamics]], [[multi-vesicular-release|multi-vesicular release]], [[multivesicular-release-spatial-information|multivesicular release as spatial information]], [[multi-vesicle-release-spatial-encoding|multi-vesicle release spatial encoding]], [[inhibitory-error-correction|inhibitory error correction]], [[inhibitory-logic-gate-pattern-cascade|inhibitory logic-gate pattern cascades]], and [[receptor-inhibition-pattern|receptor inhibition patterns]].

The perceptual and memory portions relate the recording to [[neural-phase-pattern-propagation|neural phase pattern propagation]], [[phase-differential-rendering|phase differential rendering]], [[brain-as-spectral-graph|the brain as a spectral graph]], [[fractal-oscillatory-tree|fractal oscillatory trees]], [[fractal-brain-scale-hierarchy|fractal brain scale hierarchy]], [[fractal-neural-scale-hierarchy|fractal neural scale hierarchy]], [[grid-cells-place-cells-oscillators|grid cells, place cells, and oscillators]], [[oscillatory-synchrony-multimodal-integration|oscillatory synchrony in multimodal integration]], [[neural-synchronization-bonding-analogy|neural synchronization bonding analogies]], [[oscillator-environmental-encoding|oscillator environmental encoding]], [[oscillatory-memory-invariance|oscillatory memory invariance]], and [[memory-as-regenerable-pattern|memory as a regenerable pattern]].

The rendering thread also points to [[brain-as-dissipative-system|the brain as a dissipative system]], [[consciousness-as-dissipative-self-organizing-system|consciousness as a dissipative self-organizing system]], [[brain-as-rendering-engine|the brain as a rendering engine]], [[continuous-neural-rendering|continuous neural rendering]], [[perceptual-screen|the perceptual screen]], [[neural-tomographic-lightfield-rendering|neural tomographic lightfield rendering]], [[neural-code-transmission|neural code transmission]], [[neural-code-functions|neural code functions]], and [[neural-timing-encoding|neural timing encoding]].
...
So I'm imagining an oscillator as like, as like, imagine that you are, you are rolling around, like, it's like, it's like when energy is spiking, right,
...
I'm sorry, why, why neurons eventually move towards being an oscillatory state where there is, it's because as a reaction to the [[action-potential|action potential]], it's like the [[action-potential|action potential]] is a drop in the ocean and the rest of the neurons are
...
At the top of the hierarchy, in a sense, you know, there's like, you know, the signals come in from your senses to the nuclei on the thalamus, and then they go from the thalamus to the, to the neocortex, and there's these narrow paths that go throughout the neocortex where the information goes to the
...
It won't reject patterns that, that it's not looking for because, that it's because, because it just won't see patterns that it's not looking for because those won't go to its, its, its active synaptic connections.
...
Because that will inhibit the cell, and so one [[action-potential|action potential]] will inhibit its neighbors, its neighboring cells. And so those cells like, you know, if they,
...
Yeah, it actually would be relevant, yeah, because the rush of sodium opening would cause the [[action-potential|action potential]] to start faster, and the rush of potassium opening would either inhibit the cell, or it would cause the cell to shorten the [[action-potential|action potential]].

And if you have, if the frequency of the [[action-potential|action potential]] is shorter, then the amplitude of its radius is going to be larger.
...
So if we're sending out, so the gamma ray bursts, you know, the frequencies of gamma ray, sorry, the gamma frequency of brain waves, because the brain can oscillate between like 0.02 hertz and 600 hertz.
...
Maybe even like, you know, higher frequency would be gamma. That's what I imagine that you're going to have a little bit more of that. I have to check. And then the, the, towards the, you know, the top, at the [[pyramidal-cell|pyramidal cell]] level.
...
And then, and then someone in particular came out at the one minute and 52nd mark, that just was, it was shocking to see them, because that person had been dead for three years and now they were back.
...
But even if it's like, it's a really subtle amount of energy, maybe what it's triggering is just, just enough to trigger neural activity, right? It's tracking a change, basically. So it turns energy into receiving, receiving the energy of the photon.
...
but the synaptic connections will also lead to a situation where the [[action-potential|action potential]] is going to depolarize other neurons,
...
like then, you know, I think of it as the global. So in the book, he's saying like basically like the transformation of, you know, sending a spike from one neuron to other neurons.
...
And so those neurons will be inhibited because they've been sort of like fed.
...
So the [[action-potential|action potential]] inhibits many neurons simultaneously and the, this might trigger, it should trigger an eventual oscillation, but it triggers the oscillation of the network in stages.

Because what happens is it's only going to affect like the downstream field effect of the neuron [[action-potential|action potential]].
...
So they are magnifying the, they are in a way magnifying the [[action-potential|action potential]], but magnifying it with a signature that's in the negative.
...
So the question is, is the energy distributed sort of like everywhere evenly or is, if one neuron is depressed, does that mean other neurons around the depressed area are more likely to fire?
...
And so the combined information pattern that defines the next layer, the information pattern that defines the next layer, let's say that the pattern is defined by both the spots that light up and the spots that don't light up.
...
help to clarify, you know, they help to amplify it's, it's, it's sort of like the [[attention-schema|attention schema]], right, the [[attention-schema|attention schema]], what was this thing where the brain is able to focus more clearly on the information that's coming in, because
...
That's also going to affect the tonic oscillator because the tonic, because, because in the [[tonic-oscillation|tonic oscillation]], because now you've changed, now you've changed a small oscillation, you moved a small oscillation out of sync with the global oscillation pattern.
...
So let's say that, you know, the [[action-potential|action potential]] fires. The overall, the overall thing is that the [[action-potential|action potential]] is adding in new energy to the system. And what's happening is the inhibition is part of the process of the whole system dissipating that energy in a cascade of activity.
...
The total charge of the oscillator can stay at the same, the total charge of the oscillator may not change at all. It seems possible.
...
so going back to the inhibition process that comes as a result of the [[action-potential|action potential]] is going to magnify, sort of like, it's going to, in a sense, magnify the impact of the [[action-potential|action potential]] on the oscillator, but it's an in between step between the [[action-potential|action potential]],
...
But there's, okay, so, but the in between stage, so the [[action-potential|action potential]] passes like an LTD signal to a group of neurons, right, an inhibitory signal.

So they become desynchronized in a sense, but their desynchronization as a group is a magnification of the [[action-potential|action potential]] moment.

That is, that's basically the [[attention-schema|attention schema]] mechanism.
...
So, and until, you know, so basically like the slowed down neurons are going to, it's like, it's like when you, so when an oscillator passes a phase around,
...
- Source file: `b0103ywhisper.md` in github.com/v5ma/selfawarenetworks
- Commit SHA: `d1c26a6fbf273695185a4036411215f53209eb71`
- Blob permalink: https://github.com/v5ma/selfawarenetworks/blob/d1c26a6fbf273695185a4036411215f53209eb71/b0103ywhisper.md
