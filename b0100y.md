...
I just improved the text in note b0100y a lot. It's much more readable. This note really drives home how a rendering in neural circuits can be conscious qualia in the human mind. (The note still needs a lot of work)
...
A sensory deprivation tank is like a drug free way to start to hallucinate and have the experience of seeing things that are not there. If you have no incoming data, your brain just fires without input from sound, & without input from light, so you hallucinate naturally without drugs. That experience can be incredibly therapeutic & healing. So sensory deprivation can be a good experience for some people.
...
The circuit is like one thing is two, things are making a distinction and then the third thing is observing and you can almost do it with like just two neurons, because each one of them is capable of observing that another neuron is different from itself? You could almost do that because it's like, if you had a circuit with two things and one thing could detect that it was not the same as the other thing. This distinction has to exist in space, but also in time. That's the key, as long as the distinction is a pattern that exists in space & time, then an observer can be implied, and since the neurons that are making the distinctions are also detecting the distinctions, they are making sense of their own sensations.
