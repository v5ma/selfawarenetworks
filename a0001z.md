...
# Individual Receptor Modulation
...
The message transmitted between neurons is synapse specific meaning that its not like the neuron is a single led light, which was a thought experiment, but rather it's like the individual synapses on the neuron are the LED lights.
...
The precision & granularity of pattern transmission is greater than specified in NAPOT Whitepaper Draft 3..
...
2. the idea that metabotropic receptors on the receiving dendrite might trigger cAMP messages to change individual calcium channels in the exit terminal.

3. The fact that synapses are individually inhibited or excited points to this possibility that the neurons output to its network is tailored synapse by synapse to represent memories with greater resolution & definition.

4. Sparse Distributed Memory by Pentti Kanerva: The idea of individual synaptic modification is consistent with the concept of Sparse Distributed Representation (a major topic at Numenta), meaning that only a 2 or 3 of the exit terminal synapses need to be excited, and the rest inhibited, for one neuron to pass on a very specific pattern to its exit terminal array.

5. The idea of "References Frames" in our Cortical Columns from the book A Thousand Brains may apply to the slow tonic oscillations that are brain wide. The higher layers tend to have slower delta & theta frequencies with a higher magnitude lower frequency. While the incoming sensory inputs to the lower layers of the brain seem to trend with higher brainwave frequencies, alpha, beta, and gamma frequences. Although I have read some papers that seem to contradict this hypothesis at times, more research is needed.
